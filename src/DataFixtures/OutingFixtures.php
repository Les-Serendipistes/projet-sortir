    ...
    public function load(ObjectManager $manager)
    {
        ...
        for ($o = 0; $o <= 40; $o++) {
            ...
            if ($dateTimeStart < $now) {
                $outing->setState($this->getReference(StateFixtures::STATE_REFERENCE.'_'.(5)));
            } else if ($dateTimeStart == $now) {
                $outing->setState($this->getReference(StateFixtures::STATE_REFERENCE . '_' . (4)));
            } else if ($dateTimeStart > $now){
            $outing->setState($this->getReference(StateFixtures::STATE_REFERENCE . '_' . mt_rand(1, 2)));
            }
